...
def store_snippets(snippets: list[Image]):
    ...
    for snippet in snippets:
        ...
        non_alpha_snippet = snippet.convert("RGB")
        non_alpha_snippet.save(temp_file, config.convert_type)
...
def build_snippets(document_name, page, query):
    ...
    snippets = [page_image.crop(box) for box in snippet_boxes]
    snippet_boxes = [__box_pil2pdf(box, metadata['dimensions']['scale'], metadata['dimensions']['origHeight'])
                      for box in snippet_boxes]
    return snippets, snippet_boxes, metadata
...
def __open_page_image(document_name, page):
    doc_dir = f'{config.metadata_path}/{document_name}'
